...
Mud_AppResult MudBackend_init(void* backend_options) {
    ...
    check_error(!SDL_CreateWindowAndRenderer(
        state.options.window_title,
        state.options.windowWidth,
        state.options.windowHeight,
        state.options.windowFlags,
        &state.window,
        &state.renderer
    ), "SDL_CreateWindowAndRenderer");
    check_error(!SDL_SetWindowPosition(state.window, state.options.windowX, state.options.windowY),
