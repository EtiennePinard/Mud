...
MudSDLBackend_FontData* MudSDLBackend_openFontData(MudSDLBackend_FontData* data, const char* fontPath, float pointSize, TextType textType) {
    TTF_Font* font = TTF_OpenFont(fontPath, pointSize);

    if (!font) {
        SDL_LogError(SDL_LOG_CATEGORY_ERROR, "Failed to open font at %s with error %s\n", fontPath, SDL_GetError());
        return NULL;
    }
...
Mud_AppResult findFontSizeToFit(const char* textString, SDL_FRect rectToFit, TTF_Font* baseFont, bool isTextMultiLine, float* resultingFontSize) {
...
Mud_AppResult renderSingleLineTextCenteredToFit(TTF_Font* font, const char* text, SDL_Color color, SDL_FRect rect) {
    TTF_Font* tempFont = TTF_CopyFont(font);
    if (!tempFont) return MUD_TERMINATE_WITH_FAILURE;

    float fontSizeToFit;
    if (findFontSizeToFit(text, rect, tempFont, false, &fontSizeToFit) != MUD_CONTINUE) { TTF_CloseFont(tempFont); return MUD_TERMINATE_WITH_FAILURE; }
    if (!TTF_SetFontSize(tempFont, fontSizeToFit)) { TTF_CloseFont(tempFont); return MUD_TERMINATE_WITH_FAILURE; }

    SDL_Surface* textSurface = TTF_RenderText_Blended_Wrapped(tempFont, text, 0, color, 0);
    if (!textSurface) { TTF_CloseFont(tempFont); return MUD_TERMINATE_WITH_FAILURE; }

    SDL_Texture* textTexture = SDL_CreateTextureFromSurface(state.renderer, textSurface);
    if (!textTexture) { SDL_DestroySurface(textSurface); TTF_CloseFont(tempFont); return MUD_TERMINATE_WITH_FAILURE; }
    ...
    if (!SDL_RenderTexture(state.renderer, textTexture, NULL, &renderTextRect)) return MUD_TERMINATE_WITH_FAILURE;
    TTF_CloseFont(tempFont);
...
Mud_AppResult renderMultilineTextCenteredToFit(TTF_Font* font, const char* text, SDL_Color color, SDL_FRect rect) {
    ...
    float fontSizeToFit;
    if (findFontSizeToFit(text, rect, tempFont, true, &fontSizeToFit) != MUD_CONTINUE) { TTF_CloseFont(tempFont); return MUD_TERMINATE_WITH_FAILURE; }
    if (!TTF_SetFontSize(tempFont, fontSizeToFit)) { TTF_CloseFont(tempFont); return MUD_TERMINATE_WITH_FAILURE; }

    char* textCopy = strdup(text);
    if (!textCopy) { TTF_CloseFont(tempFont); return MUD_TERMINATE_WITH_FAILURE; }
