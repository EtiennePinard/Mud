...
Mud_AppResult MudBackend_popEvent(Mud_Event *outEvent) {
  ...
  case SDL_EVENT_MOUSE_MOTION:
    ...
    // We "normalize" the pressedButtonFlag so that we stay within the buttons
    // Mud defines
    outEvent->mouseMove.pressedButtonFlag &=
        (MUD_LEFT_BUTTON_MASK | MUD_RIGHT_BUTTON_MASK | MUD_MIDDLE_BUTTON_MASK);
    break;
  ...
  case SDL_EVENT_KEY_UP:
    ...
    outEvent->key = (Mud_KeyEvent){.scancode = event.key.scancode,
