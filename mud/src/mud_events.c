...
Mud_AppResult Mud_handleEvents(Mud_App* app) {
    ...
    case MUD_EVENT_NONE:
        ...
        return result;
    case MUD_EVENT_QUIT:
        if (app->events.onQuit) { result = app->events.onQuit(&event, app); }
        else {
    ...
    case MUD_EVENT_KEY_UP:
        if (app->events.onKeyUp) { result = app->events.onKeyUp(&event, app); }
        break;
    ...
    case MUD_EVENT_MOUSE_MOVE:
        ...
        // Reverse iteration since the last layout boxes have the highest
        // z-order
