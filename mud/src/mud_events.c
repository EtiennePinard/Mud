...
Mud_AppResult Mud_handleEvents(Mud_App* app) {
    ...
    case MUD_EVENT_MOUSE_MOVE:
        Mud_Point previousMousePoint = app->events.mouseState.mousePoint;
        // Start by updating mouse state
        app->events.mouseState.mousePoint = (Mud_Point){
            .x = event.mouseMove.x,
            .y = event.mouseMove.y
        };
        app->events.mouseState.holdingLeftMouseButton = event.mouseMove.pressedButtonFlag & MUD_LEFT_BUTTON_MASK;
        app->events.mouseState.holdingRightMouseButton = event.mouseMove.pressedButtonFlag & MUD_RIGHT_BUTTON_MASK;
        ...
        for (size_t index = layout.numLayoutBox; index-- > 0;) {
            ...
            bool isLastPointInsideRect = Mud_pointInRect(
                previousMousePoint,
                layoutBox.renderRect
            );
            ...
            if (!isLastPointInsideRect && isCurrentPointInsideRect) {
                if (layoutBox.onMouseEntered) result = layoutBox.onMouseEntered(&event, app);
                if (result != MUD_CONTINUE) return result;
            }
            if (isCurrentPointInsideRect) {
                if (layoutBox.onMouseHovered) result = layoutBox.onMouseHovered(&event, app);
                if (result != MUD_CONTINUE) return result;
            }
            if (isLastPointInsideRect && !isCurrentPointInsideRect) {
                if (layoutBox.onMouseExited) result = layoutBox.onMouseExited(&event, app);
                if (result != MUD_CONTINUE) return result;
            }
    ...
    case MUD_EVENT_MOUSE_BUTTON_UP:
        ...
        if (event.mouseButton.mouseButtonIndex == MUD_LEFT_MOUSE_BUTTON) {
            app->events.mouseState.holdingLeftMouseButton = event.type == MUD_EVENT_MOUSE_BUTTON_DOWN;
        }
        else if (event.mouseButton.mouseButtonIndex == MUD_RIGHT_MOUSE_BUTTON) {
            app->events.mouseState.holdingRightMouseButton = event.type == MUD_EVENT_MOUSE_BUTTON_DOWN;
        }
    ...
    case MUD_EVENT_MOUSE_WHEEL:
        ...
        for (size_t index = layout.numLayoutBox; index-- > 0;) {
            ...
            if (isCurrentPointInsideRect) {
                switch (event.type) {
                case MUD_EVENT_MOUSE_BUTTON_DOWN:
                    if (layoutBox.onMouseButtonDown) result = layoutBox.onMouseButtonDown(&event, app);
                    goto END_INNER_SWITCH_EVENT_TYPE;
                case MUD_EVENT_MOUSE_BUTTON_UP:
                    if (layoutBox.onMouseButtonUp) result = layoutBox.onMouseButtonUp(&event, app);
                    goto END_INNER_SWITCH_EVENT_TYPE;
                case MUD_EVENT_MOUSE_WHEEL:
                    if (layoutBox.onMouseWheelScrolled) result = layoutBox.onMouseWheelScrolled(&event, app);
                    goto END_INNER_SWITCH_EVENT_TYPE;
                default:
