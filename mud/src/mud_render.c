...
Mud_AppResult Mud_render(Mud_App *app) {
  ...
  if (atomic_load(&app->rerenderLayoutNextFrame) == MUD_NO_RERENDER) {
    ...
    // If we already have a MUD_OTHER_THREAD_RERENDER value then we don't want
    // to rerender twice so we set the value to MUD_MAIN_THREAD_RERENDER
  ...
  for (size_t index = 0; index < sceneLayout.numLayoutBox; index++) {
    ...
    if (renderFunction)
      result = renderFunction(layoutBox.renderRect, app);
  ...
  // MAIN_THREAD This means that if shouldRender is MUD_NO_RERENDER then it will
  // stay that way else we go to a main thread rerender so that we rerender once
