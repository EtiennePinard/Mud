...
Mud_Rect Mud_getWindowRect() {
    return MudBackend_getWindowRect();
}
...
Mud_AppResult Mud_addPrimitive(Mud_App* app, Mud_Primitive* primitive) {
    if (app->renderQueue.count >= app->renderQueue.capacity) {
        ...
        app->renderQueue.commands = realloc(app->renderQueue.commands, app->renderQueue.capacity);
        if (app->renderQueue.commands == NULL) return MUD_TERMINATE_WITH_FAILURE;
    }
