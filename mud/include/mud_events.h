...
typedef struct Mud_MouseMoveEvent {
  Mud_EventType type;
  u32 pressedButtonFlag; // Can be AND with button mask to see if a button is
                         // pressed
  float x;               // Window x coordinates
  float y;               // Window y coordinates
} Mud_MouseMoveEvent;
...
typedef struct Mud_MouseWheelEvent {
  ...
  float y; // The amount scrolled vertically, positive away from the user and
           // negative toward the user
  Mud_MouseWheelDirection
      direction; // Set to one of the MUD_MOUSE_WHEEL_* defines. When FLIPPED
...
typedef struct Mud_KeyEvent {
  Mud_EventType type;
  u32 scancode; // The physical key pressed. Which key corresponds to which
                // number is defined by the backend
  u32 keycode;  // The key pressed by the virtual keyboard layout. Which key
                // corresponds to which number is defined by the backend
} Mud_KeyEvent;
