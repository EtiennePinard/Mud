...
#include "../../common/include/mud_common.h"
...
Mud_AppResult renderHelloWorldButton(Mud_Rect rect, Mud_App* app) {
    ...
    MudCommon_returnOnFailure(MudCommon_addFilledQuad(app, rect, sceneData->filledColor));
    MudCommon_returnOnFailure(MudCommon_addBorderQuad(app, rect, sceneData->borderColor, 1.0));
    return MudCommon_addText(app, rect, sceneData->helloWorldText, sceneData->textColor, &sceneData->fontData);
}
...
Mud_AppResult incrementCounter(Mud_Event* event, Mud_App* app) {
    ...
    snprintf(sceneData->helloWorldText, TEXT_MAX_SIZE, "Hello, World! (%u)", sceneData->counter);
...
Mud_AppResult computeInitialSceneLayout(Mud_App* app) {
    ...
    app->scene.sceneLayout.layoutBoxes[0] = (Mud_LayoutBox){
        ...
        .renderRect = MudCommon_centerRectInContainer(
            MudCommon_scaleRectToParent(parentRect, BUTTON_WIDTH_RATIO, BUTTON_HEIGHT_RATIO),
            parentRect),
...
void initScene(Mud_App* app) {
    ...
    MudSDLBackend_openFontData(&sceneData->fontData, FONT_PATH, 10.0, SINGLE_LINE);
    sceneData->counter = 0;
    snprintf(sceneData->helloWorldText, TEXT_MAX_SIZE, "Hello, World! (%u)", sceneData->counter);
    ...
    app->scene.sceneLayout.layoutBoxes = malloc(app->scene.sceneLayout.numLayoutBox * sizeof(Mud_LayoutBox));
